...
//Used GET to allow user to move between pages	
if($_SERVER["REQUEST_METHOD"] == "GET")
{
	...
	if(isset($_GET["id"]))
	{
		...
		{
			echo "<ul>";
			foreach($contacts[$_GET["id"]] as $name => $value)
			{
				echo "<li>$name : $value</li>"; 
			}
			echo "</ul>";
			
			echo "<a href='contacts.php'>Home</a>";	
		}
		else
			{
				echo "<a href='contacts.php'>Home</a>";			
		...
		echo "<ul>";
		foreach($contacts as $contactNumber => $contactArray) //
		{
			echo "<li><a href='contacts.php?id=$contactNumber'>{$contactArray['name']} - {$contactArray['city']} - {$contactArray['state']}</a></li>"; 
		}
		echo "</ul>";	
		}	
	}
